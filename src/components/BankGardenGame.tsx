...
interface Achievement {
  ...
  icon: React.ElementType;
  reward: {
    type: 'mr' | 'mutation_chance' | 'sprinkler_discount' | 'special_seed';
    amount: number;
  };
  condition: {
    type: 'harvest_count' | 'total_earned' | 'mutations_gained' | 'plants_planted' | 'consecutive_days';
    target: number;
  };
  unlocked: boolean;
  progress: number;
...
interface GardenModifier {
  ...
  icon: React.ElementType;
  effect: {
    type: 'garden_size' | 'mutation_chance';
    amount: number;
  };
  maxLevel: number;
  purchased: boolean;
...
export const BankGardenGame: React.FC = () => {
  ...
  const fetchGardenData = useCallback(async () => {
    ...
    try {
      ...
      if (error && error.code === 'PGRST116') { // No rows found, first time player
        ...
        const loadedModifiers = data.garden_modifiers || GARDEN_MODIFIERS.map(m => ({ ...m }));
  ...
  const handlePurchaseModifier = (modifierId: string) => {
    ...
    setGardenModifiers(prevModifiers => 
      prevModifiers.map(m => 
        m.id === modifierId 
    ...
    <Container maxWidth="md" sx={{ py: 4 }}>
      ...
      <Card elevation={4} sx={{ mb: 4, borderRadius: theme.shape.borderRadius, textAlign: 'center', py: 4 }}>
        <CardContent>
          ...
          {gameState === 'playing' && (
            <Box>
              <Stack direction="row" justifyContent="space-around" alignItems="center" mb={2} flexWrap="wrap">
              ...
              <Box 
                sx={{
                  ...
                  gap: 1,
                  width: 'min(100%, 500px)', // Increased max width for larger grids
          ...
          <Dialog open={modifiersDialogOpen} onClose={() => setModifiersDialogOpen(false)} maxWidth="md" fullWidth PaperProps={{ /* Removed borderRadius */ }}>
            <DialogTitle sx={{ borderBottom: '1px solid', borderColor: 'divider', pb: 1.5 }}>
              Модификаторы сада
            </DialogTitle>
            <DialogContent sx={{ pt: 2 }}>
